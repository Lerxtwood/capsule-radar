...
#define TAMAPOKE_FW_VERSION "1.2"
...
// Arduino .ino files normally get prototypes generated for them. This combined
// firmware builds TamaPoke as normal C++, so forward-declare the sketch helpers.
void ensureMon();
...
bool tamapoke_begin() {
  ...
  Serial.printf("[tamapoke] begin heap=%u largest=%u psram=%u\n",
  ...
  if (!gfx) {
    Serial.println("[tamapoke] acquiring host panel");
    Arduino_CO5300 *panel = display::rawPanel();
    if (!panel) {
      Serial.println("TamaPoke: panel no disponible");
      return false;
    }
    Serial.println("[tamapoke] creating canvas");
    gfx = new Arduino_Canvas(LCD_WIDTH, LCD_HEIGHT, panel);
    // The host radar firmware already owns and initialized the CO5300 panel.
    // Arduino_Canvas::begin() normally calls output->begin() again; doing that
    // here can clobber the live display/LVGL state during the app handoff.
    if (!gfx || !gfx->begin(GFX_SKIP_OUTPUT_BEGIN)) {
      Serial.println("TamaPoke: canvas begin fallo");
      return false;
    }
    Serial.printf("[tamapoke] canvas ready heap=%u largest=%u psram=%u\n",
                  (unsigned)ESP.getFreeHeap(),
                  (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL),
                  (unsigned)ESP.getFreePsram());
  }
  ...
  pet.begin();
  Serial.println("[tamapoke] sd begin");
  sdBegin();
  Serial.println("[tamapoke] thumbs load");
  thumbs.load();
  ...
  pet.syncClock(e);
  Serial.printf("[tamapoke] ready heap=%u largest=%u psram=%u\n",
                (unsigned)ESP.getFreeHeap(),
                (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL),
                (unsigned)ESP.getFreePsram());
...
void renderClock() {
  ...
  char ver[20];
  snprintf(ver, sizeof(ver), "TamaPoke v%s", TAMAPOKE_FW_VERSION);
  gfx->setTextSize(1);
